...
class _CodeGreenTopHeaderState extends State<CodeGreenTopHeader>
  ...
  /// Keeps the toggle's own visual state separate from the parent so
  /// structural rebuilds (pinned, actions) don't destroy the animation.
  ...
  Widget build(BuildContext context) {
    ...
    // Use the *parent's* initialValue for structural props (pinned, actions)
    // so they don't change until the parent actually rebuilds.
    final parentTab = widget.initialValue;

    return SliverAppBar(
      pinned: parentTab == MainTab.greenSquare,
      floating: false,
      snap: false,
                ...
                child: AnimatedToggleSwitch<bool>.dual(
                  ...
                  onChanged: (value) {
                    final selectedTab = value
                        ? MainTab.codeGreen
                        : MainTab.greenSquare;
                    // Update internal state so the toggle animates.
                    setState(() => _selectedTab = selectedTab);
                    // Notify parent after the animation so the structural
                    // rebuild (pinned, actions, slivers) doesn't kill it.
                    Future.delayed(
                      const Duration(milliseconds: 350),
                      () {
                        if (mounted) widget.onChanged?.call(selectedTab);
                      },
                    );
                  },
                ),
